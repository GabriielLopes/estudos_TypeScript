...
function showErrorMessage(input: HTMLInputElement, msg: string): void {
  ...
  const errorMessage = form_fields.querySelector('.info-erro') as HTMLParagraphElement;
...
function checkEmail(input: HTMLInputElement): void {
  if (!isEmail(input.value)) showErrorMessage(input, '* O e-mail informado é inválido!');
}
